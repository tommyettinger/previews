...
public class Main extends InputAdapter implements ApplicationListener {
	...
	public void create () {
		...
		ObjectFloatOrderedMap<String> m = ObjectFloatOrderedMap.with("argl-blargl!", 1/16f);
		// figure out the width and height of the koala for collision
		// detection and rendering by converting a koala frames pixel
		// size into world units (1 unit == 16 pixels)
		Koala.WIDTH = m.getAt(0) * regions[0].getRegionWidth();
		Koala.HEIGHT = m.getAt(0) * regions[0].getRegionHeight();

		// load the map, set the unit scale to 1/16 (1 unit == 16 pixels)
		map = new TmxMapLoader().load("level1.tmx");
		renderer = new OrthogonalTiledMapRenderer(map, m.getAt(0));
